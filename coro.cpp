...
struct expected_storage
{
  int _error = 0;
  union {
    const std::error_category* _error_cat = nullptr;
    T _value;
  };
...
class expected
{
  public:
    ...
    constexpr expected(std::unexpect_t, std::error_code error) noexcept
    {
    ...
    {
      std::construct_at(&_storage._value, std::forward<Args>(args)...);
    }
    ...
    {
      std::construct_at(&_storage._value, std::forward<U>(value));
    }
    ...
    {
      if (*this)
        // TODO: figure out if we want to throw, std::terminate() or assert fail here
